        ...
        private void SaveImagesToDisk(string folder)
        {
            ...
            foreach (PropertyInfo monikerName in monikers)
            {
                ...
                if (monikerName.Name[0] != letter)
                {
                    ...
                    var items = monikers.Count(m => m.Name[0] == monikerName.Name[0]);
                ...
                var fileName = Path.Combine(folder, monikerName.Name + ".png");

                var stride = glyph.PixelWidth * (glyph.Format.BitsPerPixel / 8);
                var buffer = new byte[stride * glyph.PixelHeight];
        ...
        private void SaveGlyphsToDisk(string folder)
        {
            ...
            foreach (var groupName in groups)
            {
                ...
                foreach (var itemName in items)
                {
                    ...
                    var fileName = Path.Combine(folder, group.ToString(), item.ToString() + ".png");

                    SaveBitmapToDisk(glyph, fileName);

                    var stride = glyph.PixelWidth * (glyph.Format.BitsPerPixel / 8);
                    var data = new byte[stride * glyph.PixelHeight];
